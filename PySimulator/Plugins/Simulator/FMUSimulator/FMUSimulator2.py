...
import FMUInterface2_Connected
...
class Model(SimulatorBase.Model):
    ...
    def __init__(self, modelName=None, modelFileName=None, config=None, isConnected=False, connectedfmusitems=None, xml=None, xmlFileName=None, fmiType=None, connectionorder=None):
        ...
        if self.isConnected is False:
            ...
            self.modelType = 'Connected FMU for ' + ('Model Exchange' if self.interface.activeFmiType == 'me' else 'CoSimulation')
...
#        (eventInfo, status) = self.interface.fmiInitialize(fmiTrue, errorTolerance)
            ...
            for series in mtsf._mtsf.results.series.values():
                for category in series.category.values():
                    if self.isConnected is False:
                        ...
                        category.references = FMUInterface2_Connected.createfmiReferenceVector(category.nColumn)
    ...
    def setValue(self, valueName, valueValue):
        ...
        if self.isConnected is False:
            ...
            ScalarVariableReferenceVector = FMUInterface2_Connected.createfmiReferenceVector(1)
    ...
    def getValue(self, name):
        ...
        if k > 0:
            if self.isConnected is False:
                ...
                ref = FMUInterface2_Connected.createfmiReferenceVector(k)
            for i in xrange(k):
                ref[i] = refReal[i]
